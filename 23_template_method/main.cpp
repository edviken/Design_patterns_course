...
int main() {
  ...
  Creature c1{1, 2};
  Creature c2{1, 3};
  vector<Creature> creatures{c1, c2};

  cout << "==== Temporary Game ====" << endl;
  TemporaryCardDamageGame tempGame{creatures};
  auto res = tempGame.combat(0, 1);
  cout << "No one should win. Exp: -1.  Got: " << res << endl;


  cout << "==== Permanent Game ====" << endl;
  PermanentCardDamageGame permGame{creatures};
  auto res2 = permGame.combat(0, 1);
  cout << "No one should win. Exp: -1.  Got: " << res2 << endl;
  auto res3 = permGame.combat(0, 1);
  cout << "c2 should win. Exp: 1.  Got: " << res3 << endl;
